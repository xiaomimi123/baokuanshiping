...
## 范围调整（2026-09-17 最终评审）

最终评审后决定将以下三项前端能力延后到后续迭代，本轮不实现：

- Build 产物画廊 / 内联预览（图片、视频直接在任务详情页展示）
- WhisperX 配置卡（默认停用 endpoint 的一键添加与参数表单）
- Profile 保存前 diff 预览（PUT 前展示改动对比再确认写入）

后端产物下载接口 `GET /api/builds/:id/outputs/:name` 已就绪，前端接入留待后续迭代。
